...
if(isset($_POST['form1'])) {
    // Seleccionar la información de todos los productos para poder actualizar el carrito rápidamente
    $i = 0;
    ...
    // Por cada producto agregado seleccionar su ID, cantidad agregada y nombre
    $i=0;
    ...
    for($i=1;$i<=count($arr1);$i++) {
        ...
        // Si no hubiera suficiente productos en stock, aparece un mensaje de que no hay suficientes disponibles
        if($table_quantity[$temp_index] < $arr2[$i]) {
        	$allow_update = 0;
            $error_message .= '"'.$arr2[$i].'" productos no disponibles para "'.$arr3[$i].'"\n';
        } else {
            $_SESSION['cart_p_qty'][$i] = $arr2[$i];
        }
    }
    // Por el contrario, aparece el mensaje de que se ha actualizado correctamente
    $error_message .= '\nCantidad de otros productos actualizada correctamente';
    ?>
    
    <?php if($allow_update == 0): ?>
    	<script>alert('<?php echo $error_message; ?>');</script>
	<?php else: ?>
		<script>alert('Cantidades de todos los productos actualizadas correctamente');</script>
	<?php endif; ?>
    <?php
...
<div class="page">
	<div class="container">
		<div class="row">
			<div class="col-md-12">
                <!-- Comienza el código HTML para la estructura del carrito -->
                <?php if(!isset($_SESSION['cart_p_id'])): ?>
                    <?php echo '<h2 class="text-center">Tu carrito está vacío</h2></br>'; ?>
                    <?php echo '<h4 class="text-center">Añade productos a tu carrito para visualizarlos aquí.</h4>'; ?>
                <?php else: ?>
